...
<main class="flex h-dvh w-dvw px-2 flex-col items-center">
	...
	<button class="mb-2 px-4 py-2 rounded-md outline outline-1 outline-black text-black w-fit self-center bg-gradient-to-tr from-teal-200 to-teal-100" on:click={() => goto('/recipes/create')}
		>Write a new recipe</button
	>
	<div class="flex gap-2 flex-wrap mx-2">
		{#each userRecipes as recipe}
			<button type="button" class="p-2 grow outline outline-1 outline-black text-black bg-gradient-to-tr from-teal-200 to-teal-100 shadow-lg" on:click={() => goto(`/recipes/${recipe.recipe_id}`)}>	
